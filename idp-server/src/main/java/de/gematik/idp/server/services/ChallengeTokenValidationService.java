...
public class ChallengeTokenValidationService {
  ...
  public void validateChallengeToken(final JsonWebToken signedChallenge) {
    final Set<String> amr =
        signedChallenge
            .getBodyClaim(ClaimName.AUTHENTICATION_METHODS_REFERENCE)
            .filter(List.class::isInstance)
            .map(List.class::cast)
            .stream()
            .flatMap(List<String>::stream)
            .collect(Collectors.toSet());
    final boolean isAltAuth =
        amr.containsAll(Arrays.asList("mfa", "hwk"))
            && (amr.contains("fpt")
                || amr.contains("face")
                || amr.contains("pin")
                || amr.contains("pwd")
                || amr.contains("generic-biometric")
                || amr.contains("kba"));
    if (isAltAuth) {
      try {
  ...
  private void validateAlternateAuthenticationDataAndThrowExceptionIfFail(
      final JsonWebToken signedAuthData) {
    final X509Certificate authDataCert =
        signedAuthData
    ...
    final String keyIdentifier =
        signedAuthData
            ...
            .orElseThrow(
                () ->
                    new IdpServerException(
                        "Unable to find key identifier in authentication data",
                        IdpErrorType.INVALID_REQUEST,
                        HttpStatus.BAD_REQUEST));
    ...
    final PairingData pairingData =
        pairingRepository
            ...
            .orElseThrow(
                () ->
                    new IdpServerException(
                        "Unable to find pairing entry with given id-number and key-identifier",
                        IdpErrorType.INVALID_REQUEST,
                        HttpStatus.BAD_REQUEST));
    ...
    final DeviceInformation deviceInformation =
        retrieveDeviceInformationFromAuthData(signedAuthData);
    ...
    if (deviceValidationService
      ...
      throw new IdpServerException(
          ...
          IdpErrorType.INVALID_REQUEST,
          HttpStatus.BAD_REQUEST);
    } else if (deviceValidationService
            .assess(deviceInformation.getDeviceType())
            .equals(DeviceValidationState.UNKNOWN)
        && pairingData.getTimestampPairing().isBefore(ZonedDateTime.now().minusMonths(6))) {
      throw new IdpServerException(
          "Device validation failed. Pairing expired!",
          IdpErrorType.INVALID_REQUEST,
          HttpStatus.BAD_REQUEST);
    }
    final JsonWebToken signedPairingDataFromDto =
        new JsonWebToken(pairingData.getSignedPairingData());
    signedPairingDataFromDto.verify(
        retrieveKeyFromPairingDto(pairingData, AUTH_CERT_SUBJECT_PUBLIC_KEY_INFO));
    validateCertSn(
        authDataCert,
        signedPairingDataFromDto
            .getStringBodyClaim(CERTIFICATE_SERIALNUMBER)
            .orElseThrow(
                () ->
                    new IdpServerException(
                        "CertID not found in pairing data",
                        IdpErrorType.INVALID_REQUEST,
                        HttpStatus.BAD_REQUEST)));
  ...
  private PublicKey retrieveKeyFromPairingDto(
      final PairingData pairingData, final ClaimName claimName) {
    return new JsonWebToken(pairingData.getSignedPairingData())
        ...
        .orElseThrow(
            () ->
                new IdpServerException(
                    "PublicKey not found in pairing data",
                    IdpErrorType.INVALID_REQUEST,
                    HttpStatus.BAD_REQUEST));
  }
  ...
  public String getIdNumberFromAuthDataCertClaims(final X509Certificate authCert) {
    return Optional.ofNullable(
            X509ClaimExtraction.extractClaimsFromCertificate(authCert)
                .get(ClaimName.ID_NUMBER.getJoseName()))
  ...
  private DeviceInformation retrieveDeviceInformationFromAuthData(
      final JsonWebToken signedAuthData) {
    return signedAuthData
        .getBodyClaim(DEVICE_INFORMATION)
        ...
        .orElseThrow(
            () ->
                new IdpServerException(
                    "Device information not found in auth data",
                    IdpErrorType.INVALID_REQUEST,
                    HttpStatus.BAD_REQUEST));
  }
