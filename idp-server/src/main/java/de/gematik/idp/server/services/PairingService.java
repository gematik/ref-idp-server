...
public class PairingService {
  ...
  private static final Set<ClaimName> SIGNED_PAIRING_DATA_CLAIMS =
      Set.of(
  ...
  public void validateTokenAndDeleteSelectedPairing(
      final JsonWebToken accessToken, final String keyIdentifier) {
    validateAccessTokenClaims(accessToken);
  ...
  public void deleteSelectedPairing(final String idNumber, final String keyIdentifier) {
    final long result = pairingRepository.deleteByIdNumberAndKeyIdentifier(idNumber, keyIdentifier);
    if (result == 0) {
      throw new IdpServerException(
          4000,
          IdpErrorType.INVALID_REQUEST,
          "Der Auftrag zur Deaktivierung des Pairings konnte nicht angenommen werden.",
  ...
  public PairingDto validatePairingData(
      final JsonWebToken accessToken, final IdpJwe encryptedRegistrationData) {
    final RegistrationData registrationData =
        decryptAndValidateRegistrationData(encryptedRegistrationData);
    ...
    final X509Certificate authCert =
        CryptoLoader.getCertificateFromPem(
            Base64.getUrlDecoder().decode(registrationData.getAuthCert()));
    checkIdNumberIntegrity(authCert, idNumber);
    checkSignedPairingDataClaims(signedPairingData);
    try {
      authenticationChallengeVerifier.verifyResponseWithCertAndThrowExceptionIfFail(
          authCert, signedPairingData);
    ...
    if (deviceValidationService
        .assess(deviceInformation.getDeviceType())
        .equals(DeviceValidationState.BLOCK)) {
      throw new IdpServerException(
          IdpServerException.ERROR_ID_BLOCKLIST,
          IdpErrorType.ACCESS_DENIED,
          "Device validation matched with not allowed devices!",
          HttpStatus.BAD_REQUEST);
    }
    final PairingData data =
        createPairingDtoFromRegistrationData(
            signedPairingData, idNumber, deviceInformation.getName());
  ...
  private void checkSignedPairingDataClaims(final JsonWebToken signedPairingData) {
    final Optional<ClaimName> missingClaim =
        SIGNED_PAIRING_DATA_CLAIMS.stream()
  ...
  public PairingData insertPairing(final PairingData pairingData) {
    if (pairingRepository
        .findByIdNumberAndKeyIdentifier(pairingData.getIdNumber(), pairingData.getKeyIdentifier())
        .isPresent()) {
      throw new IdpServerException(
          4004,
          IdpErrorType.INVALID_REQUEST,
          "Pairing for this ID/Key-ID combination already in DB",
          HttpStatus.CONFLICT);
    }
        ...
        .orElseThrow(
            () ->
                new IdpServerException(
                    "idNumber not found in accessToken",
                    IdpErrorType.INVALID_REQUEST,
                    HttpStatus.BAD_REQUEST));
  }
  ...
  private void validateAccessTokenClaims(final JsonWebToken accessToken) {
    if (accessToken.getBodyClaim(AUTHENTICATION_METHODS_REFERENCE).isEmpty()) {
      throw new IdpServerException(
          "Claim amr not found in accessToken",
          IdpErrorType.INVALID_REQUEST,
          HttpStatus.BAD_REQUEST);
    }
    if (accessToken.getBodyClaim(AUTHENTICATION_CLASS_REFERENCE).isEmpty()) {
      throw new IdpServerException(
          "Claim acr not found in accessToken",
          IdpErrorType.INVALID_REQUEST,
          HttpStatus.BAD_REQUEST);
    }
  }
  ...
  private PairingData createPairingDtoFromRegistrationData(
      final JsonWebToken signedPairingData, final String idNumber, final String deviceName) {
    return PairingData.builder()
        .id(null)
        .idNumber(idNumber)
        .keyIdentifier(
            signedPairingData
                .getStringBodyClaim(KEY_IDENTIFIER)
                .orElseThrow(
                    () ->
                        new IdpServerException(
                            "Key identifier not found in pairing data",
                            IdpErrorType.INVALID_REQUEST,
                            HttpStatus.BAD_REQUEST)))
  ...
  private void checkIdNumberIntegrity(final X509Certificate authCert, final String idNumber) {
    try {
      final Map<String, Object> certClaims =
          X509ClaimExtraction.extractClaimsFromCertificate(authCert);
      ...
      if (!idNumber.equals(idNumberCert)) {
        throw new IdpServerException(
            IdpServerException.ERROR_ID_ACCESS_DENIED,
            IdpErrorType.ACCESS_DENIED,
            "IdNumber does not match to certificate!",
            HttpStatus.FORBIDDEN);
      }
      ...
      throw new IdpServerException(
          IdpServerException.ERROR_ID_ACCESS_DENIED,
          IdpErrorType.ACCESS_DENIED,
          "Error while extracting claim from certificate!",
          HttpStatus.FORBIDDEN);
    }
  }

  private String getIdNumberFromCertClaimsAndThrowExceptionIfNotExists(
      final Map<String, Object> certClaims) {
    final Optional<String> idNumber =
        Optional.ofNullable(certClaims.get(ClaimName.ID_NUMBER.getJoseName()))
            .filter(String.class::isInstance)
            .map(String.class::cast);
    return idNumber.orElseThrow(
        () ->
            new IdpServerException(
                IdpServerException.ERROR_ID_ACCESS_DENIED,
                IdpErrorType.ACCESS_DENIED,
                "Information ID_NUMBER not found in certificate",
                HttpStatus.FORBIDDEN));
  }

  public Optional<PairingDto> getPairingDtoForIdNumberAndKeyIdentifier(
      final String kvnr, final String keyIdentifier) {
