...
class IdpAuthenticatorTest {
  ...
  void validateRedirectUriWithNullValue_ExpectCorrectError() {
    assertThatThrownBy(
            () -> idpAuthenticator.validateRedirectUri(TestConstants.CLIENT_ID_E_REZEPT_APP, null))
  ...
  void validateRedirectUriWithNonValidClientId_ExpectCorrectError() {
    final IdpClientConfiguration idpClientConfiguration =
        idpConfiguration.getRegisteredClient().get(TestConstants.CLIENT_ID_E_REZEPT_APP);
    assertThatThrownBy(
            () ->
                idpAuthenticator.validateRedirectUri(
                    "test", idpClientConfiguration.getRedirectUri()))
        .isInstanceOf(IdpServerException.class)
        .hasFieldOrPropertyWithValue("errorType", IdpErrorType.INVALID_REQUEST);
  }
  ...
  void validateRedirectUriWithInvalidValue_ExpectCorrectError() {
    assertThatThrownBy(
            () ->
                idpAuthenticator.validateRedirectUri(TestConstants.CLIENT_ID_E_REZEPT_APP, "test"))
        .isInstanceOf(IdpServerException.class)
        .hasFieldOrPropertyWithValue("errorType", IdpErrorType.INVALID_REQUEST);
  }
  ...
  void validateRedirectUriIsEqualToConfigurationValue() {
    final IdpClientConfiguration idpClientConfiguration =
        idpConfiguration.getRegisteredClient().get(TestConstants.CLIENT_ID_E_REZEPT_APP);
    idpAuthenticator.validateRedirectUri(
        TestConstants.CLIENT_ID_E_REZEPT_APP, idpClientConfiguration.getRedirectUri());
  }
  ...
  void validateAuthorizationCodeLocation() {
    ...
    final String authorizationCodeLocation =
        idpAuthenticator.getAuthorizationCodeLocation(idToken, sessionMap);
    assertThat(authorizationCodeLocation)
        .startsWith("http://userAgentRedirectUri")
        .contains("code=ey")
