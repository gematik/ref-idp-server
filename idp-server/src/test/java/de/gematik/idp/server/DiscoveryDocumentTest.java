...
class DiscoveryDocumentTest {
  ...
  @Afo("A_20458")
  @Remark(
      "Die Afo gibt keine vollstaendige Liste der zu verwendenden Attribute. Das ergibt sich mit"
          + " dem Rfc und anderen Abschnitten der Spec")
  ...
  void testValueForSubjectTypesSupported() throws UnirestException {
    final List<String> subjectTypesSupported =
        (List)
            extractClaimMapFromResponse(retrieveDiscoveryDocument()).get("subject_types_supported");
    assertThat(subjectTypesSupported).containsExactlyInAnyOrder("pairwise");
  }
  ...
  @Test
  @Remark(
      "Ruecksprache mit der Spec hat zu BP256R1 gefuehrt, weil es zuvor keinen Bezeichner fuer"
          + " ECDSA mit brainpool256r1 bei JWS gab")
  ...
  void testValueForGrantTypesSupported() throws UnirestException {
    final List<String> grantTypesSupported =
        (List)
            extractClaimMapFromResponse(retrieveDiscoveryDocument()).get("grant_types_supported");
  ...
  void testValueForAcrValuesSupported() throws UnirestException {
    final List<String> acrValuesSupported =
        (List<String>)
            retrieveAndParseDiscoveryDocument().getBodyClaim(ClaimName.ACR_VALUES_SUPPORTED).get();
  ...
  @Rfc("https://openid.net/specs/openid-connect-discovery-1_0.html")
  @Remark(
      "bei uns findet keine Auth am Token Endpoint statt, daher none, Parameter vorhanden weil"
          + " Defaultwert HTTP-Basic-Auth wäre")
  ...
  void testValueForIssuer() throws UnirestException {
    final String issuer =
        (String) extractClaimMapFromResponse(retrieveDiscoveryDocument()).get("issuer");
  ...
  @Afo("A_20458")
  @Remark(
      "nach ruecksprache mit Tommy wird nicht die Bezeichnung aus der afo, sondern die von oidc"
          + " vorgesehene verwendet")
  void testValueForAuthorizationEndpoint() throws UnirestException {
    final String authorizationEndpointValue =
        extractClaimMapFromResponse(retrieveDiscoveryDocument())
  ...
  void testValueForTokenEndpoint() throws UnirestException {
    final String tokenEndpointValue =
        extractClaimMapFromResponse(retrieveDiscoveryDocument()).get("token_endpoint").toString();
    assertThat(tokenEndpointValue).isEqualTo(CONFIGURED_SERVER_URL + TOKEN_ENDPOINT);
  }

  @Test
  @Afo("A_20458")
  void testValueForJwksUri() throws UnirestException {
    assertThat(retrieveAndParseDiscoveryDocument().getStringBodyClaim(ClaimName.JWKS_URI).get())
  ...
  void testValueExpiration() throws UnirestException {
    assertThat(retrieveAndParseDiscoveryDocument().getExpiresAtBody())
        .isBetween(
            ZonedDateTime.now().minusMinutes(1).plusHours(24), ZonedDateTime.now().plusHours(24));
  }
  ...
  void testValueForCodeChallengeMethodsSupported() throws UnirestException {
    ...
    assertThat(codeChallengeMethodsSupported).containsExactlyInAnyOrder("S256");
  }

  @Test
  @Afo("A_20591")
  void testDiscoveryDocumentSigningCertificateReference() throws UnirestException {
    assertThat(retrieveAndParseDiscoveryDocument().getHeaderClaim(ClaimName.X509_CERTIFICATE_CHAIN))
        .isPresent();
  }
