...
class X509ClaimExtractionTest {
  ...
  private static final String KRANKENHAUS_FILE =
      "src/test/resources/Krankenhaus_min_R2048_X509.pem";

  @Test
  void extractFromEgk() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFromP12(EGK_FILE));
    assertThat(claims)
  ...
  void extractFromSmcbApotheke() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFromP12(SMCB_FILE));
    assertThat(claims)
  ...
  void extractFromSmcbArztpraxis() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFrom(ARZTPRAXIS_FILE));
    assertThat(claims)
  ...
  void extractFromSmcbKrankenhaus() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFrom(KRANKENHAUS_FILE));
    assertThat(claims)
        .containsEntry("given_name", null)
        .containsEntry("family_name", null)
  ...
  void extractFromHbaArzt() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFrom(HBA_CERT_FILE));
    assertThat(claims)
  ...
  void extractFromHbaPsychtherapeut() throws IOException, CertificateEncodingException {
    final Map<String, Object> claims =
        X509ClaimExtraction.extractClaimsFromCertificate(certificateDataFrom(PSYCHOTHERAPEUT_FILE));
    assertThat(claims)
  ...
  // TODO Testcase extractFromHbaPsychtherapeut is wip, open question:
  // https://projekt-jira.int.gematik.de/browse/IDP-98

  private byte[] certificateDataFromP12(final String filename)
  ...
  private byte[] certificateDataFrom(final String filename)
      throws IOException, CertificateEncodingException {
    return CryptoLoader.getCertificateFromPem(FileUtils.readFileToByteArray(new File(filename)))
        .getEncoded();
  }
}
