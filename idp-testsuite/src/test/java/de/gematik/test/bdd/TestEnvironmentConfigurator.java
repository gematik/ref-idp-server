 ...
 * startup. ${TESTENV.xxxxx} in feature files values/params/tables will be replaced with the value of the property
 * TESTENV.xxxxx in the configured testsuite config properties file.
 */
@Slf4j
public class TestEnvironmentConfigurator {
    ...
    private static void initialize() {
        ...
        try (final FileInputStream fis = new FileInputStream(configFile)) {
            ...
            throw new AssertionError("Unable to load test suite configuration for test environment '" + testEnv + "'",
                e);
        }
        log.debug("  substituting env variables referenced in properties...");
        props.entrySet().forEach(entry -> entry.setValue(substituteEnvironmentVariables(entry.getValue().toString())));
        props.entrySet().stream().sorted(Comparator.comparing(left -> left.getValue().toString()))
