...
public class IdpTestEnvironmentConfigurator extends TestEnvironmentConfigurator {
    ...
    private static final String DISCOVERY_URL_TEMPLATE = "http://idp" + /*127.0.0.1:%d*/ "/.well-known/openid-configuration";
    private static final String AUTHORIZATION_URL_SEKTORAL_IDP_TEMPLATE = "http://idpsektoral" + /*127.0.0.1:%d*/ "/authorization";
    ...
    public static synchronized void initializeIDPTestEnvironment() {
        ...
        final int serverPort = Integer.parseInt(TigerGlobalConfiguration.resolvePlaceholders(TigerGlobalConfiguration.readString("tiger.ports.idp")));
        final int sektoralIdpPort =  Integer.parseInt(TigerGlobalConfiguration.resolvePlaceholders(TigerGlobalConfiguration.readString("tiger.ports.idpsektoral")));
        ...
        if (idpLocalDiscdoc == null || idpLocalDiscdoc.isBlank()) {
            if (getProperty(IDP_SERVER) != null) {
                ...
                AUTHORIZATION_URL_SEKTORAL_IDP = String.format(AUTHORIZATION_URL_SEKTORAL_IDP_TEMPLATE,
                    sektoralIdpPort);
            }
        } else {
            try {
