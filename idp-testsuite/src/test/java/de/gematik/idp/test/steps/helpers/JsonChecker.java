...
public class JsonChecker {
  ...
  public void assertJsonResponseHasNode(final String path) throws JSONException {
    assertThatJson(new JSONObject(Context.getCurrentResponse().getBody().asString()))
        .node(path)
        .isNotNull();
  }
                ...
                (oracleJson, testJson) -> {
                  if (testJson instanceof JSONObject) {
                    ...
                    assertJsonArrayShouldMatchInAnyOrder(
                        testJson.toString(), oracleJson.toString());
                    return true;
                  } else {
  ...
  public void assertJsonShouldMatchInAnyOrder(final String jsonStr, final String oracleStr) {
    ...
    try {
      ...
      json.keySet()
          .forEach(
              key ->
                  assertThat(finalOracle.has(key) || finalOracle.has("____" + key))
                      .withFailMessage("EXTRA Key " + key + " in JSON")
                      .isTrue());
      ...
      while (keyIt.hasNext()) {
        ...
        if (!"${json-unit.ignore}".equals(oracleValue)) {
          if (json.get(jsonKey) instanceof JSONObject) {
            assertJsonShouldMatchInAnyOrder(
                json.get(jsonKey).toString(), oracle.get(oracleKey).toString());
          } else if (json.get(jsonKey) instanceof JSONArray) {

            JSONAssert.assertEquals(
                oracle.get(oracleKey).toString(), json.get(jsonKey).toString(), customComparatpr);
          } else {
      ...
      Assertions.fail(
          String.format(
              "JSON does not match!\nExpected:\n%s\n\n--------\n\nReceived:\n%s",
              oracle == null ? null : oracle.toString(2), json == null ? null : json.toString(2)),
          nsme);
  ...
  final CustomComparator customComparatpr =
      new CustomComparator(
          JSONCompareMode.LENIENT,
          new Customization(
              "***",
  ...
  @Step
  public void assertJsonResponseHasExactlyOneNodeAt(final String node, final String path)
      throws JSONException {
  ...
  @Step
  @SneakyThrows
  public void assertJsonShouldMatch(
      final SerenityJSONObject serenityJSONObject, final String claimName, final String regex) {
  ...
  @Step
  @SneakyThrows
  public void assertJsonShouldNotMatch(
      final SerenityJSONObject serenityJSONObject, final String claimName, final String regex) {
