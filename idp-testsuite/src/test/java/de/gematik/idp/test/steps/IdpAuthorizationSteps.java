...
public class IdpAuthorizationSteps extends IdpStepsBase {
  ...
  public void signChallenge(final String keyfile, final String password) {
    ...
    Context.get()
        .putString(ContextKey.SIGNED_CHALLENGE, jsonWebSignature.getCompactSerialization());
  }
  ...
  public void getCode(final CodeAuthType authType, final HttpStatus expectedStatus)
    ...
    final Response r =
        requestResponseAndAssertStatus(path, null, HttpMethods.POST, params, null, expectedStatus);
  ...
  @SneakyThrows
  private String checkParamsNGetPath(
      final CodeAuthType authType, final Map<String, String> params) {
    ...
    switch (authType) {
      case SIGNED_CHALLENGE:
        ...
        final Optional<Pair<String, Object>> expHeader =
            extractExpHeader(params.get("signed_challenge"));
        if (expHeader.isPresent()) {
          params.put(
              "signed_challenge",
              keyAndCertificateStepsHelper.encrypt(
                  "{\"njwt\":\"" + params.get("signed_challenge") + "\"}",
      ...
      case ALTERNATIVE_AUTHENTICATION:
        checkContextAddToParams(
            ContextKey.SIGEND_AUTHENTICATION_DATA, "encrypted_signed_authentication_data", params);
        // encrypt signed challenge
        params.put(
            "encrypted_signed_authentication_data",
            keyAndCertificateStepsHelper.encrypt(
                "{\"njwt\":\"" + params.get("encrypted_signed_authentication_data") + "\"}",
                DiscoveryDocument.getPublicKeyFromContextKey(ContextKey.PUK_ENC),
                Pair.of("cty", "NJWT"),
                Pair.of("typ", "JWT"),
  ...
  @NotNull
  private Optional<Pair<String, Object>> extractExpForAltAuthChallenge(
  ...
  private void storeResponseContentInContext(
    ...
    assertThat(reloc)
        .withFailMessage("No Location header in response", r.getHeaders())
        .isNotBlank();

    ctxt.put(ContextKey.TOKEN_REDIRECT_URL, reloc);

    // if token encryption active decrypt code and store it in context
    final Optional<String> encTokenCode =
        storeParamOfReloc(reloc, "code", ContextKey.TOKEN_CODE_ENCRYPTED);
    assertThat(encTokenCode).withFailMessage("Encrypted token code not found").isPresent();
    final CodeAuthType tokenSecret =
        authType == CodeAuthType.ALTERNATIVE_AUTHENTICATION
    ...
    if (authType != CodeAuthType.SSO_TOKEN) {
      ...
      final Optional<String> encSsoToken =
          storeParamOfReloc(reloc, "ssotoken", ContextKey.SSO_TOKEN_ENCRYPTED);
      try {
        Context.get()
            .put(
  ...
  private Optional<String> storeParamOfReloc(
      final String reloc, final String paramName, final String key) throws URISyntaxException {
    final Optional<String> value =
        new URIBuilder(reloc)
            .getQueryParams().stream()
  ...
  private void checkContextAddToParams(
      final String key, final String paramName, final Map<String, String> params) {
    assertThat(Context.get().getMapForCurrentThread())
        .containsKey(key)
        .doesNotContainEntry(key, null);
    params.put(paramName, String.valueOf(Context.get().get(key)));
  }

  public void responseIsErrorWithMessageMatching(
      final int httpStatus, final int errid, final String errcode) {
